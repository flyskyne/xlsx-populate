...
class XmlParser {
    ...
    parseAsync(xmlText) {
        return new externals.Promise((resolve, reject) => {
            ...
            parser.onopentagstart = node => {
                // If root element, find it's namespace and remove.
                let nodeName = node.name;
                if (!current) {
                    const rootNamespace = this._extractNamespace(node);
                    if (rootNamespace === 'x') {
                        rootNamespaceRegExp = new RegExp(`^${rootNamespace}:`);
                    }
                }
                if (rootNamespaceRegExp) {
                    nodeName = node.name.replace(rootNamespaceRegExp, '');
                }
                const child = { name: nodeName, attributes: {}, children: [] };
                if (current) {
     ...
     * @private
     */
    _extractNamespace(node) {
        const delimiterIndex = node.name.indexOf(':');
        if (delimiterIndex > -1) return node.name.substr(0, delimiterIndex);
    }
}
